...
import (
	...
	"github.com/foadmom/yari/common"
	"github.com/foadmom/yari/comms"
	"github.com/foadmom/yari/message"
...
func MainFlowManager(components *types.RootStructure) {
	...
	if components.Internal.Role == types.ROLE_Follower {
		go followerHeartbeatManager(components)
	}
	if components.Internal.Role == types.ROLE_Leader {
		go leaderHeartbeatManager(components)
	}
...
func requestCampaignReset(components *types.RootStructure, resources *campaignManagerResources) error {
	...
	if _err != nil {
		startANewCampaign(components, resources)
	}
...
func (ballot ballotBox) addVote(candidate string, majorityNeeded int) (int, bool) {
	...
	if _candidateCard.Votes >= majorityNeeded {
		_hasTheNumbers = true
	}
